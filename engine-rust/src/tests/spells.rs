...
fn test_mana_vault_etb_sets_doesnt_untap() {
    ...
    // Cast Mana Vault
    state.apply_action(&crate::action::Action::CastSpell { card_id: vault_id, targets: vec![], x_value: 0, from_graveyard: false, from_library_top: false, alt_cost: None, modes: vec![] }, &db);
